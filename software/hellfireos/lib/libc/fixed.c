...
fixed_t fix_square(fixed_t arg){
	fixed_t temp;
	temp = fix_mul(arg, arg);
	return temp;
}

//-----------------> FUNCTION TO PRINT FIXED POINT NUMBERS <--------------------

void fix_print(fixed_t arg)
{
	char buf[20];
	fixtoa(arg, buf, 6);
	printf("%15s\n", buf);
...
//------------> FUNCTION TO SHOW SIGNAL FROM A FIXED POINT NUMBER <-------------
//------------> RESULT = 1 IF N >= 0 AND RESULT = -1 IF N < 0  <----------------

fixed_t fix_sign(fixed_t arg1)
{
	fixed_t out;
	if (arg1 >= 0){
		out = fix_val(1);
...
fixed_t fix_norm(int size, fixed_t vec[])
{
	fixed_t aux, out;

	for(int i = 0; i < size; i++) {
		aux += fix_square(vec[i]);
	}
...
fixed_t cpsign(fixed_t arg1, fixed_t arg2){
	fixed_t out;
	if (arg2 >= 0){
		if (arg1 >= 0){
			out = arg1;
...
fixed_t norm_vector(int size, fixed_t vec[]){
	fixed_t aux, sum, out;
	sum = fix_val(0);
