...
//Add for multiplication instruction
USignal<uint32_t>* THellfireProcessor::GetSignalCounterMul(){
	return this->_counter_imul;
}
...
void THellfireProcessor::InitCounters(
		...
		uint32_t mul_counter_addr, //Add for multiplication instruction
    ...
    //Add for multiplication instruction
  this->_counter_imul = new USignal<uint32_t>(mul_counter_addr, GetName() + ".counters.imul");
...
*/
void THellfireProcessor::UpdateCounters(int opcode, int funct3, int funct7){
	...
	switch(opcode){
		...
		case 0x33:
			switch(funct3){
				case 0x0: //addi, subi
					switch(funct7){
					    case 0x1: //Add for multiplication instruction
					        _counter_imul->Inc(1);
									break;
...
fail:
	...
	#ifdef HFRISCV_ENABLE_COUNTERS
	this->UpdateCounters(opcode, funct3, funct7);
	#endif /*HFRISCV_ENABLE_COUNTERS*/
